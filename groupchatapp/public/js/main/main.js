    ...
    chatHistory.forEach((ele) => {
        ...
        if (ele.userId == userId) {
            ...
            <div class="col-12 mb-2 pe-0">
                ...
                messageText += `                            
                <div class="col-12 mb-2 pe-0">
                    <div class="card p-2 float-end rounded-4 self-chat-class" style="background-color: #adb5bd; color: white;">
                        <p class="text-muted my-0"><small>${ele.name}</small></p>
                        <p class="my-0">${ele.message}</p>
                        <small class="text-secondary text-end">${formattedDate}</small>
                    </div>
                </div>`
            }
        } else {
            if(ele.isImage){
                ...
                messageText += `                            
                <div class="col-12 mb-2 pe-0">
                    <div class="card p-2 float-start rounded-4 chat-class" style="background-color: #ced4da; color: #212529;">
                        <p class="text-muted my-0"><small>${ele.name}</small></p>
                        <p class="my-0">${ele.message}</p>
    ...
    try {
        ...
        <button class="list-group-item list-group-item-action py-2" 
            ...
            <div class="d-flex w-100 align-items-center justify-content-between" id="0">
                ...
                <strong class="mb-1">HI-WORLD!</strong>
    ...
    try {
        ...
        if (chats) {
            ...
            if (parsedChatHistory.length > 0) {
                const lastMessageId = parsedChatHistory[parsedChatHistory.length - 1].messageId || 0;
...
async function setupGroup(groupId, userId) {
    try {
        if (groupId == 0) {
            ...
            group_heading.innerHTML = `HELLO-WORLD!`;
